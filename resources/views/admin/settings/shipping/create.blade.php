...
@section('page-title', 'Tambah Metode Pengiriman')
@section('page-description', 'Tambahkan metode pengiriman baru')

@section('content')
<div class="max-w-3xl">
    <form action="{{ route('admin.shipping-methods.store') }}" method="POST" class="bg-white rounded-xl shadow-sm border border-neutral-200 p-6">
        @csrf
        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="3" 
                          class="w-full px-4 py-2.5 border border-neutral-300 rounded-lg focus:border-orange-600 focus:ring-2 focus:ring-orange-100 transition"
                          placeholder="Deskripsi singkat metode pengiriman">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" name="base_cost" value="{{ old('base_cost', 0) }}" min="0" step="1000"
                       class="w-full px-4 py-2.5 border border-neutral-300 rounded-lg focus:border-orange-600 focus:ring-2 focus:ring-orange-100 transition"
            ...
            <div>
                ...
                <input type="number" name="estimated_days" value="{{ old('estimated_days', 1) }}" min="0" max="30"
                       class="w-full px-4 py-2.5 border border-neutral-300 rounded-lg focus:border-orange-600 focus:ring-2 focus:ring-orange-100 transition"
